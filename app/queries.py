...
async def add_new_network(graph):
    ...
    for node in graph.nodes(data=True):
        ...
        if node[1][TYPE] == CENTER_HUB:
            ...
            new_network = models.Network(id=network_id, name=network_name)
        ...
        if node[1][TYPE] == REGION_HUB:
            ...
            if tower_list:
                for node_id in tower_list:
                    tower_node = [
                        node for node in graph.nodes(data=True) if node[0] == node_id
                    ]
...
async def calculate_total_cost(network_id, cost_details):
    ...
    center_hub_total_cost = (
        len(
            list(
                filter(lambda node: node[1][TYPE] == CENTER_HUB, graph.nodes(data=True))
            )
        )
        * cost_details[CENTER_HUB][COST]
    )
    region_hub_total_cost = (
        len(
            list(
                filter(lambda node: node[1][TYPE] == REGION_HUB, graph.nodes(data=True))
            )
        )
        * cost_details[REGION_HUB][COST]
    )
    tower_total_cost = (
        len(list(filter(lambda node: node[1][TYPE] == TOWER, graph.nodes(data=True))))
        * cost_details[TOWER][COST]
    )
    ...
    for edge in graph.edges(data=True):
        ...
        if (
            source_node[0][1][TYPE] == CENTER_HUB
            and target_node[0][1][TYPE] == REGION_HUB
        ):
...
async def get_edge_list(network_id):
    edge_list = (
        await models.Edge.select(SOURCE_NODE, TARGET_NODE)
        .where(models.Edge.network_id == network_id)
        .gino.all()
    )
    return edge_list
...
async def load_networks_with_tower_nodes(location_geo_point):
    query = (
        models.Node.outerjoin(models.Network)
        .select()
        .where(
            or_(
